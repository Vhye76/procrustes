...
A ROW'S SOURCE PATH FOLLOWS ITS FILE.  'source_path' is unique across rows, so a row must never claim a path its file has left:  '_quarantine' and '_retire' set it to the quarantine destination beside 'quarantine_path'.  A later title can then be held to, or released to, a name an earlier one used, and a new file at a quarantined or retired title's old path is a new row;  the reappearance branch meets only a row whose file is still at that path, a DRY_RUN quarantine or a failed retire.
