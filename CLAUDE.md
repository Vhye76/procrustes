...
'app/' is a single Python package with two third-party dependencies and no more.  The standard library is sufficient for everything else and the HTTP client is hand-rolled on urllib.  KEEP IT THAT WAY.  No requests, no npm, no framework, no CDN.  A dependency-free image is trivially auditable and never breaks on a transitive upgrade.

THE TWO EXCEPTIONS ARE NAMED, AND EACH DOES ONE THING THE STANDARD LIBRARY CANNOT.  'argon2-cffi' is argon2id password hashing;  'hashlib' has scrypt and pbkdf2 and no Argon2.  'qrcode' renders the authenticator enrolment as an SVG;  the standard library has no QR encoder.  Both are Alpine community packages, 'py3-argon2-cffi' and 'py3-qrcode', installed like ffmpeg and never fetched from an index at build or run time;  the build gate in section 22 asserts both import.  The CI validate job installs the same two from PyPI at the same versions, because 'import app.main' runs there on a bare runner.  'app/auth.py' is the only module that imports either.
...
A GROUP NAMES THE PAGE THAT RENDERS IT.  Each 'GROUPS' entry carries a page, 'settings' or 'account', 'describe()' emits it, and each page renders the groups that name it.  The Access group ('auth_enabled', 'session_hours') renders at the top of User Settings and is absent from Application Settings.  'auth_enabled' has one write route, 'POST /api/account/auth', because switching it off needs a signed-in user and a fresh factor;  'Settings.update' and 'reset' refuse it from '/api/settings' with 'set from the User Settings page', and 'Auth' writes it with 'internal=True'.  Section 30 carries the rules.
...
Packages:  ffmpeg, mkvtoolnix, python3, py3-argon2-cffi, py3-qrcode, bash, coreutils, findutils, jq, ca-certificates, tini, su-exec, shadow, libcap, libcap-utils, libva, libva-utils, intel-media-driver, libvpl, onevpl-intel-gpu.  All from Alpine's main and community repositories;  nothing non-free, nothing from a third-party repository, nothing static, nothing from PyPI.  'py3-qrcode' depends on 'py3-pillow' at the package level;  the SVG path the code uses never imports it.
...
AND THAT 'argon2' AND 'qrcode' IMPORT, with argon2id available.  'app/auth.py' imports both at module level, so without them 'import app.main' fails and the container never starts;  the gate says why at build time instead.
...
'x.0.0' is a release.  '0.x.0' is the implementation of new features.  '0.0.x' is a bug fix.  The current version is 0.11.0.
...
Section 3's rule, stated here as a decision so it does not erode:  every dependency added is a supply chain, an upgrade treadmill and an audit burden on an image that otherwise consists of Alpine packages and this repository.  'argon2-cffi' and 'qrcode' are the two exceptions, each admitted for one capability the standard library lacks, and each is an Alpine package rather than a pip install so the image stays a set of distribution packages plus this repository.  A third needs the same case made:  a capability the standard library cannot supply, and an Alpine package that supplies it.

### The web UI requires a login, over HTTPS only

Section 30 carries the mechanism.  The decision here is that the login is in-app and optional:  the first visit to a fresh store asks whether to require authentication, the answer is a setting, and switching it off opens the dashboard and the API to anyone who can reach the port, which is the state every version before 0.11.0 ran in.  Off is a logged degradation with a header pill, in the shape of DRY_RUN, not a configuration error.

TLS DOES NOT CHANGE WHO CAN USE THE API.  The listener is HTTPS on 443 with no HTTP listener and no redirect;  what TLS buys is that the traffic, the password and the session cookie are not readable in transit, and the cookie is marked Secure so a browser never sends it over plain HTTP.
...
THE LOGIN IS IN FRONT OF EVERY PAGE AND EVERY API ROUTE BUT FIVE.  'GET /api/health' answers '{ok, version}' for the Docker healthcheck and nothing else;  'GET /api/login' says whether authentication is on and whether the first account is still to be created;  'POST /api/setup' and 'POST /api/setup/totp' serve the first run;  'POST /api/login' takes credentials.  Everything else, '/api/settings' and the posters included, answers 401 '{"error": "login required"}' without a valid session, and the three pages serve 'login.html' in place of themselves.  'webui.Handler._user' is the one gate:  the session from the cookie, or the synthetic operator while authentication is off.

THE FIRST RUN IS ONE STATE, 'auth_enabled' ON WITH NO USER ROW.  A fresh store is that state by construction, default True and an empty table, and switching authentication on with no account produces it again;  both land on the same page.  The page asks 'Require Authentication?'.  No stores 'auth_enabled' off and opens the dashboard.  Yes takes a username and a method, Password, OTP/Google Authenticator or MFA with both, enrols the authenticator inline for the two that need it, and 'Auth.setup' verifies every factor the chosen mode needs before the row is written, so a wrong code creates nothing.  'user_create' checks the count inside the store lock, so two first-run posts cannot both succeed.  No account is seeded and no default password exists anywhere.  The window between first start and the first answer is open to whoever reaches the port;  accepted, because a seeded credential would be a wider exposure than an empty table.

THREE MODES PER USER, AND THE ROW'S MODE IS WHAT THE LOGIN CHECKS.  'password' needs a password hash, 'totp' needs a confirmed secret, 'both' needs both, and 'auth.mode_allowed' refuses a mode the row cannot satisfy.  The login form is one form for every mode, username, password and code, and the server ignores a factor the row does not require, so the page never reveals which factors a username needs.  Every failure answers 401 with the same body, 'login failed';  the reason, unknown user, wrong password, wrong or reused code, goes to the log with the username and the address.  An unknown username is verified against a dummy hash so the two paths cost the same.

ARGON2ID AT THE LIBRARY DEFAULTS, RECORDED AND NOT RETUNED.  'argon2.PasswordHasher()' as shipped in 25.1.0:  argon2id, three passes, 64 MiB, four lanes, a 32-byte hash and a 16-byte salt.  A stored hash that no longer matches those parameters is rehashed on the next successful password login.  64 MiB is allocated per verification, so concurrent logins cost memory;  the lockout bounds it.

TOTP IS RFC 6238 OVER RFC 4226 IN THE STANDARD LIBRARY, AND A CODE IS ACCEPTED ONCE.  HMAC-SHA1, six digits, a 30 second step, a window of one step either side, a 160-bit secret from 'secrets.token_bytes' in unpadded base32.  'auth.verify_code' returns the step it accepted and refuses any step at or below the row's 'last_totp_step', so the code that just signed in cannot sign in again inside its window;  the comparison is 'hmac.compare_digest'.  Verified against the RFC 6238 test vectors.  The secret is stored as it must be, in the clear in 'state.db', since the server has to compute the same code;  the store's protection is the config mount's permissions.

THE ENROLMENT TRAVELS INLINE, NEVER IN A URL.  'auth.enrolment' returns the secret, the otpauth URI and the QR as an SVG string in one JSON body, for the first run and for the account page alike, and the page inserts the SVG.  A URL carrying the secret would land in the handler's debug log line.  The QR is 'qrcode''s 'SvgPathImage';  Pillow is a package dependency of 'py3-qrcode' and is never imported.

A SESSION IS A RANDOM TOKEN THE STORE KNOWS ONLY BY ITS HASH.  'secrets.token_urlsafe(32)' in the cookie, its SHA-256 in 'sessions', with an expiry of 'session_hours' from login;  a row past its expiry is dropped on the next read, logout deletes it, a password change deletes the user's other sessions, and removing a user drops theirs.  The cookie is 'procrustes_session' with 'Path=/; Secure; HttpOnly; SameSite=Strict'.  'SameSite=Strict' plus JSON-only POST bodies is the CSRF guard:  a cross-site form post cannot carry the cookie, and a cross-site script cannot read it.  Nothing else is added.

FIVE FAILURES IN FIFTEEN MINUTES LOCK THE USERNAME AND THE ADDRESS.  'auth.Lockout' is in memory, keyed both ways, and answers 429 with 'Retry-After' for the rest of the window;  a correct login inside it is still refused.  A restart clears it, which is accepted:  the lockout bounds the argon2 cost and the guessing rate, not the store.

EVERY CREDENTIAL CHANGE RE-CHECKS A FACTOR IN THE SAME REQUEST.  Changing the mode, changing the password, removing the authenticator and switching authentication off each take the current password or a fresh code, whichever the account has, through 'Auth._confirm';  a fresh code advances 'last_totp_step' like a login.  Enrolling an authenticator writes 'totp_pending' and nothing else until a code confirms it.  Removing the authenticator is refused while the mode needs it.  Any signed-in user adds and removes users;  a user cannot remove their own account, which is what keeps at least one account in the table.

THE SWITCH HAS ONE ROUTE AND TWO DIRECTIONS.  'POST /api/account/auth' is the only writer of 'auth_enabled'.  Off needs a signed-in user and a factor and logs who did it at warning.  On has no precondition, since the caller is not signed in while it is off:  the response sets no cookie, and the next load is the login form with users present or the first-run page with none.  Sessions are not touched in either direction;  an unexpired session from before the switch is valid again once it is back on.  While off, the User Settings page shows the switch and nothing per user, every per-user and user-management endpoint answers 409, the header carries the 'AUTH OFF' pill, Sign out is hidden from the menu, and startup logs a warning.

THE DASHBOARD AND THE SETTINGS PAGE REACH THE LOGIN THROUGH A 401.  Every 'fetch' on both pages goes through one wrapper that reloads the page on 401, and the reload lands on 'login.html', which sends the browser back to the path it was on.  User Settings is a third page, 'account.html' at '/account', in the shape of the settings page.
