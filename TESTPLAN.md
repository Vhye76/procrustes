...
EVERY API CALL OUTSIDE SECTION 15 CARRIES A SESSION.  Since 0.11.0 every '/api/' route but '/api/health' and '/api/login' answers 401 without one.  Before the first case of a run, answer the first-run question with Yes, create a password account, sign in with 'curl -sk -c jar -X POST -H "Content-Type: application/json" -d '{"username": "op", "password": "..."}' https://localhost/api/login', and send '-b jar' on every later call;  a case that says "with no cookie" means exactly that.  Section 15 starts from its own state and says so.
...
- **T-70  HTTPS answers.**  'curl -sk https://localhost/api/health'.  Expect JSON with 'ok' true;  '/api/status' with the session cookie likewise.
...
- **T-72  The dashboard is served.**  'curl -sk -b jar https://localhost/'.  Expect the dashboard HTML;  without the cookie expect the login form's HTML instead.
...
Every case in this group starts from an empty state database unless it says otherwise, and reads the cookie with 'curl -sk -c jar' and sends it with '-b jar'.  A code is computed on the workstation from the secret the enrolment returned, with any RFC 6238 implementation ('oathtool --totp -b <secret>', or the reference algorithm by hand).

- **T-182  A fresh store asks the question.**  Start on an empty state database.  Expect 'GET /api/login' to answer '{"enabled": true, "setup": true}', 'GET /' to serve the login page, the page to show 'Require Authentication?' with Yes and No, and the startup log to carry 'no account exists yet'.
- **T-183  No opens the dashboard.**  POST '/api/setup' with '{"require": false}'.  Expect 200, 'GET /' to serve the dashboard with no cookie, 'GET /api/titles' to answer 200 with no cookie, the header to show 'AUTH OFF', the menu to hide Sign out, 'auth_enabled' to read stored and off on '/api/settings', and a second POST to '/api/setup' to answer 409.
- **T-184  Yes creates the first account in each method.**  Three fresh stores.  Password:  POST '/api/setup' with '{"require": true, "username": "op", "mode": "password", "password": "correct horse"}';  expect 200 with a 'Set-Cookie' and '/api/account' reading mode 'password'.  OTP:  POST '/api/setup/totp' with the username, compute the code from the returned secret, POST '/api/setup' with mode 'totp', the secret and the code;  expect 200 and mode 'totp'.  MFA:  the same with mode 'both' and a password;  expect mode 'both'.  In every case expect '/api/login' to read 'setup' false afterwards and the log line 'first account op created in mode <mode>'.
- **T-185  A wrong enrolment code creates nothing.**  On a fresh store POST '/api/setup' with mode 'totp', a secret from '/api/setup/totp' and the code '000000'.  Expect 400 naming the code, no 'Set-Cookie', '/api/login' still reading 'setup' true, and no user row.
- **T-186  Login in each mode.**  With the three accounts from T-184:  'op' with the password alone, the OTP account with a code alone, the MFA account with both.  Expect 200 with 'Set-Cookie' carrying 'Secure', 'HttpOnly', 'SameSite=Strict', 'Path=/' and 'Max-Age=604800', and 'GET /api/status' with the cookie to carry 'auth.user' as the username.  Send the MFA account the password alone and expect 401.
- **T-187  Every failure reads the same.**  POST '/api/login' with a wrong password, with a wrong code and with an unknown username.  Expect each to answer 401 with the body '{"error": "login failed"}' and nothing else, and the log to carry the three different reasons.
- **T-188  The sixth failure locks out.**  Five wrong passwords for one username from one address, then the correct one.  Expect the sixth request to answer 429 with a 'Retry-After' header and 'retry_after' in the body, the correct login refused with the same 429, and the log line 'locked out user=op'.  From a second address expect the correct login to answer 200, since that address has no failures;  a sixth wrong attempt against a different username from the first address must answer 429 on the address key.
- **T-189  A code is accepted once.**  Sign in with a code, then sign in again inside the same 30 second step with the same code.  Expect the second to answer 401 and the log to read 'wrong or reused code';  the next step's code must answer 200.
- **T-190  The gate covers every route.**  With no cookie expect 'GET /api/titles', 'GET /api/settings', 'POST /api/settings', 'GET /api/status' and 'GET /api/poster/<any>' to answer 401 with '{"error": "login required"}', and 'GET /settings' and 'GET /account' to serve the login page.  Expect 'GET /api/health' to answer 200 with 'ok' true and the running version, and 'docker inspect --format {{.State.Health.Status}}' to read 'healthy' after the start period.
- **T-191  The enrolment carries the QR inline.**  Signed in, POST '/api/account/totp/enrol'.  Expect 'secret', 'uri' and 'qr' in the body, 'qr' starting '<svg', the URI of the form 'otpauth://totp/procrustes:<user>?secret=<secret>&issuer=procrustes&algorithm=SHA1&digits=6&period=30', and the SVG, rasterised and decoded with zbarimg on the workstation, to read exactly that URI.  Expect no request in the debug log to carry the secret in a path.
- **T-192  The mode invariants hold.**  On a password-only account POST '/api/account/mode' with 'totp';  expect 400 'needs an authenticator enrolled and confirmed first' and '/api/account' to list that reason under 'modes.totp'.  Enrol, confirm with a code, then POST 'totp' with the current password;  expect 200 and the mode changed.  POST '/api/account/totp/disable' on that account;  expect 400 naming the mode.  POST the mode back to 'password' with a code, then disable with the password;  expect 200 and 'totp_enrolled' false.
- **T-193  A credential change needs a factor.**  POST '/api/account/mode', '/api/account/password' and '/api/account/totp/disable' each without a password or a code.  Expect 400 'the current password or a fresh authenticator code is required' on each and nothing changed.  POST '/api/account/password' with the correct current password and a new one from a second session's cookie jar;  expect 200, the second jar's cookie to answer 401 afterwards, and the new password to sign in.
- **T-194  Users are added and removed within the rules.**  POST '/api/users' with a username and password;  expect the new user in '/api/account' 'users' and able to sign in in password mode.  POST '/api/users/<own name>/delete';  expect 400 'cannot remove their own account'.  Remove the added user from the first account and expect 200, and the removed user's cookie to answer 401.
- **T-195  Sessions outlive a restart and end on logout.**  Sign in, restart the container, and reuse the cookie;  expect 200.  POST '/api/logout' and reuse it;  expect 401.  Set 'session_hours' to 1 on User Settings and sign in;  expect 'Max-Age=3600' on the new cookie and the earlier cookie unaffected.
- **T-196  The switch has one route.**  POST '/api/settings' with '{"set": {"auth_enabled": false}}';  expect 400 with 'errors.auth_enabled' reading 'set from the User Settings page' and nothing written.  POST '/api/account/auth' with '{"enabled": false}' and no factor;  expect 400.  With the password;  expect 200, the log line 'authentication switched off by op' at warning, 'GET /api/titles' to answer 200 with no cookie, the header pill 'AUTH OFF', and every '/api/account/*' route but 'auth' to answer 409.
- **T-197  Switching on lands where the store says.**  From the T-196 state POST '/api/account/auth' with '{"enabled": true}';  expect 200 with no 'Set-Cookie', 'GET /' to serve the login form, and the old cookie to sign in again if unexpired.  On a store with authentication off and no user row, POST the same;  expect 'GET /' to serve the first-run question.
